...
public class BorrowerUi {
	...
	System.out.println("You have selected the book "+userBook.getTitle());
	 ...
	 Loans loan = new Loans(bookId, branchId, cardNo, dateOut,dueDate,null);
		BorrowerService bS = new BorrowerService();
		if(bS.doesLoanExist(loan)) {
			bS.updateOldLoan(loan);
		}
		else {
		...
		////Display all checked out books
		 ...
		 List<Loans> userLoans = new ArrayList<>();
			...
			for(Loans aLoan : lList) {
				...
				if(aLoan.getBranchId()==userBranch.getBranchId() && bor.getCardNo()==aLoan.getCardNo()&&aLoan.getDateIn() == null) {
					//&&aLoan.getDateIn().compareTo(dateIn)!= 1 
					//if(aLoan.getDateIn() == null)
					userLoans.add(aLoan);
				}
			}
